...
// Simple constants - no external imports needed
const CREDIT_FORCE_TERMINATION_THRESHOLD = 25;
const CREDIT_CRITICAL_WINDOW_MS = 3 * 60 * 1000;
...
export const CallActive = ({ onLeave, conversationName }: CallActiveProps) => {
  ...
  useEffect(() => {
    ...
    const checkCredits = () => {
      try {
        // Simple credit calculation - no need for complex constants
        const elapsedMinutes = (Date.now() - startTime) / (1000 * 60);
        const videoCostPerMin = 0.8; // Simple constant
        const transcriptionCostPerMin = 7.2; // Simple constant  
        const processingCost = 150; // Simple constant
