...
import { Search, Sparkles, Bot, FolderOpen, Zap } from 'lucide-react';
...
export const DashboardCommand = ({ open, setOpen }: Props) => {
  ...
  const hasSessionResults = sessions && sessions.items.length > 0;
  ...
  return (
    <CommandDialog open={open} onOpenChange={setOpen}>
      <CommandInput 
        placeholder="Search agents, sessions, or start a conversation..." 
        value={search}
      ...
      <CommandList className="max-h-[60vh]">
        {showLoading ? (
          ...
          <>
            {hasAgentResults && (
              <CommandGroup heading="Agents" className="px-2 py-3">
                {agents?.items.map((agent) => (
                  ...
                  >
                    ...
                    <div className="flex items-center space-x-3 flex-1">
                      <div className="w-8 h-8 bg-gradient-to-br from-green-500 to-green-600 rounded-full flex items-center justify-center text-sm font-bold text-white shadow-sm group-hover:shadow-md group-hover:scale-105 transition-all duration-200">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-muted-foreground group-hover:text-muted-foreground/80 transition-colors duration-200">
                          {agent.category} • {agent.conversationCount} conversations
                        </p>
                      </div>
                    </div>
            ...
            {!hasAgentResults && !hasSessionResults && <EmptyState />}
          </>
        ) : (
          <>
